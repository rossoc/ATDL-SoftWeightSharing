...
# -*- coding: utf-8 -*-
"""
Test script: WideResNet-16-4 on CIFAR-10 with soft weight sharing and GIF visualization
"""
...
from src.ATDLSoftWeightSharing.models import WideResNet16x4
from src.ATDLSoftWeightSharing.data import get_cifar_data
...
class GIFVisualizer:
    ...
    def capture_weights(self, epoch, test_accuracy=None):
        ...
        ax.set_title(f'Epoch: {epoch}, Test Acc: {test_accuracy:.4f}' if test_accuracy else f'Epoch: {epoch}')
        ax.set_xlim(-0.8, 0.8)
        ax.set_ylim(-0.8, 0.8)
        ...
        for i, mean_val in enumerate(mu_np[1:11]):  # Plot first 10 means
            ax.axvline(x=mean_val, color='r', linestyle='--', alpha=0.5, linewidth=1)
...
def main():
    print("=== WideResNet-16-4 on CIFAR-10 with soft weight sharing and GIF visualization ===")
    ...
    # Create model
    print("Creating WideResNet-16-4 model...")
    model = WideResNet16x4(num_classes=10, k=4)
    model.build(input_shape=(None, 32, 32, 3))  # Specify input shape
    
    # Pretraining for a few epochs
    ...
    # Create visualizer
    visualizer = GIFVisualizer(model, prior, save_path="wideresnet164_weights_evolution.gif")
    ...
    def train_step(x_batch, y_batch):
        ...
        trainable_vars = model.trainable_variables + [prior.mu, prior.log_sigma2, prior.pi_logits, prior.log_sigma2_0]
    ...
    for epoch in tqdm(range(epochs), desc="Training with visualization"):
        epoch_loss = 0
        num_batches = 0
        ...
        print(f"Epoch {epoch + pretrain_epochs + 1}/{epochs + pretrain_epochs}, Average Loss: {avg_loss:.4f}, Test Accuracy: {test_accuracy:.4f}")
    
    # Save the GIF
    visualizer.save_gif(duration=1.0)
    print(f"Weight evolution GIF saved as wideresnet164_weights_evolution.gif")
    ...
    # Save the model
    model.save("./scripts/wideresnet164_model.keras")
    print("Model saved as wideresnet164_model.keras")
    
    print("WideResNet-16-4 training with visualization completed successfully!")
